# Crypto Portfolio Analyzer — Standalone Real GUI Implementation
...
Default URL: `http://127.0.0.1:9112`

## What is inside this project folder

- `app/` — FastAPI backend for this project.
- `static/` — elegant browser GUI.
- `plugins/crypto-portfolio-analyzer.json` — this project’s own feature/customization/input schema.
...
- Domain: `Finance / Crypto Portfolio`
...
- `risk_profile` — risk profile (select)
- `exchanges` — exchanges (text)
- `tax_jurisdiction` — tax jurisdiction (select)
- `stablecoin_policy` — stablecoin policy (text)
- `target_allocation` — target allocation (text)
...
- `holdings` — Holdings (text) required
